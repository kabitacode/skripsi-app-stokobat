         ...
         <aside id="logo-sidebar" className="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
            <div className="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
               <ul className="space-y-2 font-medium">
                  ...
                  <li className={`py-1 ${activeLink === '/obat' || '/obat/add' ? 'bg-blue-700 text-white' : 'bg-white text-gray-700'}`}>
                     <Link href="/obat" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white  dark:hover:bg-gray-700">
                        <span className={`py-1 ${activeLink === '/obat' || '/obat/add' ? 'text-white ml-3' : 'ml-3'}`} >Daftar Obat</span>
                     </Link>
                  </li>
               </ul>
            </div>
